...
export async function PATCH(req: NextRequest) {
    try {
        const { id, status, motorista, veiculo, obs_motorista, dataRemarcacao} = await req.json();

        const formattedDate = new Date(dataRemarcacao).toLocaleDateString('pt-BR', {
            day: '2-digit',
            month: '2-digit',
            year: 'numeric',
          ...
          const agendamento = await prisma.agendamentos.update({
            ...
            data: {
              ...
              veiculo,
              obs_motorista,
              dataRemarcacao: formattedDate, // Update with formatted date
            },
          });
